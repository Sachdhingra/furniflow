...
// In-memory user store for auth (sync with Supabase users table)
const users = new Map();
...
async function syncUsers() {
  try {
    ...
    users.clear();
    data?.forEach(u => users.set(u.email, u));
    console.log('Users synced:', users.size);
  } catch (e) {
    console.log('User sync error:', e.message);
  }
}
...
async function initDB() {
  try {
    ...
    console.log('Tables may need setup:', e.message);
  }
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    const u = data;
    const token = jwt.sign({ id: u.id, name: u.name, role: u.role }, SECRET, { expiresIn: '7d' });
    res.json({ token, user: { id: u.id, name: u.name, email: u.email, role: u.role, phone: u.phone } });
  } catch (e) {
...
app.post('/leads', authenticate, async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('leads')
      .insert([{ id, customerName, phone, address, productCategory, leadValue: leadValue || 0, leadDate, nextFollowUp, remarks, status: status || 'lead', createdBy: req.user.id, isSiteLead: isSiteLead || false, locked: true }])
      .select();
    if (error) throw error;
...
app.put('/leads/:id', authenticate, async (req, res) => {
  const { customerName, phone, address, productCategory, leadValue, status, nextFollowUp, remarks } = req.body;
  try {
    const { error } = await supabase
      .from('leads')
      .update({ customerName, phone, address, productCategory, leadValue, status, nextFollowUp, remarks })
      .eq('id', req.params.id);
    if (error) throw error;
...
app.post('/services', authenticate, async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('services')
      .insert([{ id, customerName, phone, address, serviceType, serviceValue: serviceValue || 0, requestDate: requestDate || new Date().toISOString().split('T')[0], attendDate, remarks, status: 'pending', createdBy: req.user.id }])
      .select();
    if (error) throw error;
    res.json({ id: data[0].id });
  } catch (e) {
...
app.get('/jobs', authenticate, async (req, res) => {
  try {
    let query = supabase.from('jobs').select('*').order('scheduledDate', { ascending: false });
...
app.post('/jobs', authenticate, async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('jobs')
      .insert([{ id, leadId, customerName, phone, address, productCategory, jobType, paymentStatus, status: 'created', createdAt, scheduledDate }])
      .select();
    if (error) throw error;
    res.json({ id: data[0].id });
  } catch (e) {
...
app.put('/jobs/:id', authenticate, async (req, res) => {
  ...
  try {
    const { error } = await supabase
      .from('jobs')
      .update({ status, assignedTo, scheduledDate, acceptedAt, travelStartAt, reachedAt, completedAt, images, remarks, incompleteReason })
      .eq('id', req.params.id);
    if (error) throw error;
    res.json({ success: true });
  } catch (e) {
...
app.post('/claims', authenticate, async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('claims')
      .insert([{ id, serviceId, customerName, phone, bookingDate, partNumber, reason, dueDate, status: 'pending', createdBy: req.user.id }])
      .select();
    if (error) throw error;
    res.json({ id: data[0].id });
  } catch (e) {
...
app.put('/claims/:id', authenticate, async (req, res) => {
...
// Site Visits
app.get('/sitevisits', authenticate, async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('siteVisits')
      .select('*')
...
app.post('/trips', authenticate, async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('trips')
      .insert([{ id, userId: req.user.id, date, startTime, status: 'active' }])
      .select();
    if (error) throw error;
    res.json({ id: data[0].id });
  } catch (e) {
...
app.get('/trips/active', authenticate, async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('trips')
      .select('*')
...
app.post('/sitevisits', authenticate, async (req, res) => {
  const { tripId, customerName, phone, address, visitDate, visitTime, visitType, payment, photos, remarks, gpsLat, gpsLng } = req.body;
  const id = generateId();
  try {
    const { data, error } = await supabase
      .from('siteVisits')
      .insert([{ id, tripId, customerName, phone, address, visitDate: visitDate || new Date().toISOString().split('T')[0], visitTime, visitType, payment: payment || 0, photos, remarks, gpsLat, gpsLng, createdBy: req.user.id, locked: false }])
      .select();
    if (error) throw error;
    res.json({ id: data[0].id });
  } catch (e) {
...
app.post('/users', authenticate, async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('users')
      .insert([{ id, name, email, phone, password: password || 'changeme', role, active: true }])
      .select();
    if (error) throw error;
    res.json({ id: data[0].id });
  } catch (e) {
...
app.put('/users/:id', authenticate, async (req, res) => {
  ...
  try {
    const { error } = await supabase
      .from('users')
      .update({ name, email, phone, role, active, ...(password && { password }) })
      .eq('id', req.params.id);
    if (error) throw error;
    res.json({ success: true });
  } catch (e) {
...
app.get('/dashboard', authenticate, async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      todayVisits: siteVisits.data?.length || 0
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
