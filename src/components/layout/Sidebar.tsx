  ...
  return (
    <>
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 p-2 bg-[#1A1A2E] border border-[#3A3A5C] rounded-lg text-white"
      >
      ...
      >
        ...
        <button
          ...
          className="hidden lg:flex absolute -right-3 top-1/2 transform -translate-y-1/2 w-6 h-6 bg-[#2A2A44] border border-[#3A3A5C] rounded-full items-center justify-center text-[#A0A0B8] hover:text-white hover:border-[#E94560] transition-all"
        >
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-[#1A1A2E] border-t border-[#3A3A5C] z-30 px-2 py-1">
          ...
          {filteredNavItems.slice(0, 4).map(item => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex flex-col items-center py-2 px-3 ${isActive ? 'text-[#E94560]' : 'text-[#A0A0B8]'}`}
              >
                {item.icon}
